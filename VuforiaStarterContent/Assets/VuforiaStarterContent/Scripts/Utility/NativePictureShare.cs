...
public class NativePictureShare : MonoBehaviour
{
	private string m_ScreenshotName = "Screenshot.png";

    public void ShareScreenshotWithText(string text)
    {
        string screenShotPath = Application.persistentDataPath + "/" + m_ScreenshotName;
        if(File.Exists(screenShotPath)) File.Delete(screenShotPath);

        Application.CaptureScreenshot(m_ScreenshotName);

        StartCoroutine(delayedShare(screenShotPath, text));
    }

    public void ShareScreenShot(string pathPicture)
    {
        StartCoroutine(delayedShare(pathPicture, ""));
    } 
